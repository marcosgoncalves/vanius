;(function($){
	...
	$(document).ready(function(){
		...
		if(!$('.cactus-sidebar-control').hasClass('sb-ct-small')){
			...
			$('.cactus-open-left-sidebar a, .open-sidebar-small').on('click', function(){
				...
				if($('body').hasClass('open-mobile-sidebar-ct') && $smallSiderbar.length>0) {
					if($('body').width() > 1275){
					$('.ct-small').animate({marginLeft:-200},350);
					}
					// $('.main-content-col').css('width','75%');

					if($('.cactus-sidebar-content', $smallSiderbar).hasClass('mCustomScrollbar')) {

						if($('body').width() > 1275){
							if(window.location.href!=domain)
							$('.main-content-col').css('width','65%');
							else
							$('.main-content-col').css('width',$('body').width());
						}
					...
					$('.cactus-sidebar-content', $smallSiderbar).addClass('position-absolute').mCustomScrollbar({theme:'minimal-dark',});
						...
						// $('.main-content-col').css('width','65%');
						if($('body').width() > 1275){
							$('.ct-small').animate({marginLeft:0},350);
						}

						if($('body').width() > 1275){
							if(window.location.href!=domain)
							$('.main-content-col').css('width','55%');
							else
							$('.main-content-col').css('width',$('.cactus-container').width()-180);
						}
				}
		...
		/*sidebar-small*/
		var domain = 'http://sebrae.forsocios.com/wpforsocios/'
		if(window.location.href!=domain && $('body').width()>1275)
			$('.cactus-open-left-sidebar a').click()
...
jQuery(document).ready(function($) {
		...
		// 	e.preventDefault()
		// 	window.open($('#login').attr('href'), "nome", "width='500', height='300', scrollbars='yes', location=no, directories=no, status=no, menubar=no, toolbar=no, resizable=yes");
		// })
